...
const PROMPT = `Create a 1920x1080 landscape hero background image for a robotics framework website. LIGHT MODE version.

Art direction: Renaissance Da Vinci composition principles (golden ratio, Vitruvian proportions, radial symmetry from a focal point) — BUT rendered in modern minimalist line art. Think Leonardo's notebook sketches reimagined as a modern tech wireframe on parchment-white paper.
...
Color palette:
- Clean white to pale lavender background (#F5F5FA to #EAEAF2 gradient)
- Lines in deep teal (#008866) at very low opacity (8-15%)
- Accent touches of warm amber (#CC8800) at 5-8% opacity
- Faint indigo (#4A4AFF) hints at 3-5% opacity
...
- The feel should be: intellectual, elegant, precise, like opening a Da Vinci codex in bright daylight

ABSOLUTELY NO TEXT, no words, no letters, no numbers, no labels.`;

async function generateHero() {
  fs.mkdirSync(OUT_DIR, { recursive: true });
  const outFile = path.join(OUT_DIR, "hero-bg-light.png");

  console.log("Generating light hero image with Gemini...");
    ...
    try {
      ...
      for (const part of parts) {
        if (part.inlineData) {
          ...
          console.log(`Light hero saved: ${outFile} (${(buf.length / 1024).toFixed(0)}KB)`);
          return outFile;
        }
      }
